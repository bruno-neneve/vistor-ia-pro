...
if api_key:
    try:
        ...
        # Nome do modelo exatamente como funcionou anteriormente
        ...
        if uploaded_files:
            if st.button("🚀 Gerar Laudo Técnico"):
                ...
                header_text = f"""**INSPEÇÃO VISUAL PRELIMINAR - Vistor.IA Pro** **Data da Inspeção:** {data_hoje} | **Cliente:** {nome_cliente} | **Endereço:** {endereco_imovel}  
---
A presente análise baseia-se exclusivamente na imagem fornecida, não sendo possível realizar testes destrutivos ou medições precisas. As classificações de estado (🟢🟡🔴) referem-se à condição aparente na imagem."""
                ...
                for uploaded_file in uploaded_files:
                    ...
                    # Instruções específicas de engenharia solicitadas
                ...
                # Resumo Geral Consolidado (Item 6)
                st.divider()
                st.subheader("📝 Resumo Geral do Imóvel")
                with st.spinner("Consolidando inteligência..."):
                    prompt_resumo = f"Escreva um 'Resumo' técnico final. Destaque conservação, padrão e cite patologias críticas encontradas nas fotos: \n{contexto_consolidado}"
    ...
    st.info("Insira sua API Key na lateral para começar.")
